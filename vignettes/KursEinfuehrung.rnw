...
\frame{
  ...
  \frametitle{Einf\"uhrung in R - Erste Schritte}
  \begin{block}{Installation von R}
    \begin{itemize}
    \item Download von CRAN (Comprehensive R Archive Network): https://stat.ethz.ch/CRAN/
    \item Installation des Binaries (.exe unter Windows und .pkg unter Mac) mit Doppelclick auf Datei
    \item Webseite https://www.r-project.org/
    \end{itemize}
  \end{block}

  \begin{block}{Installation von RStudio}
    \begin{itemize}
