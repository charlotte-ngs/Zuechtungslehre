...
\frame{
  \frametitle{Einf\"uhrung in R - Installation}
  %\begin{block}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
  \item \emph{Nat\"urliche Selektion} (NS): Umwelt bestimmt, welche Tiere in einer Population \"uberleben (survival of the fittest).
  \item NS f\"uhrt zu einer Akkumulation von g\"unstigen Eigenschaften (im Bezug auf gewisse Umweltbedingungen), welche \"uber lange Zeit zu neuen Lebensformen f\"uhren (Darwinsche Selektionstheorie)
  \item \emph{K\"unstliche Selektion} (KS): Mensch bestimmt, welche Tiere sich fortpflanzen und wie lange die Tiere in einer Population verbleiben.
  \item Durch KS werden (f\"ur Menschen) g\"unstige Eigenschaften (Genotype) gef\"ordert und ung\"unstige verhindert.
  \item Ersatz von NS durch KS ist ein Charakteristikum der Domestikation und eine Ursache der Entstehung der Haustiere aus den Wildtieren
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
  \item \emph{Zuf\"allige Paarung} (Panmixie): Jedes Tier in der Population hat die gleiche Chance sich zu paaren
  \item \emph{Gerichtete Paarung} (assortative Paarung): Mensch bestimmt, welche Tiere einer Population sich paaren
