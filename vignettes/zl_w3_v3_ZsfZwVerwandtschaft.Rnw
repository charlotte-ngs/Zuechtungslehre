...
\frame{
  ...
  \begin{itemize}
  \item Populationsmittel $\mu$ als Erwartungswert $E\left[g\right]$ der genotypischen Werte
  \item Allgemeine Definition des Erwartungswertes einer diskreten Zufallsvariablen $X$
  $$E\left[X\right] = \sum_{x\in \mathcal{X}} x* f(x)$$
...
\frame{
  \frametitle{Populationsmittel Genotypfrequenzen}
  \begin{itemize}
  \item Gewichtung der genotypischen Werte aufgrund der Vorkommensh\"aufigkeit
  \item $\mu = {1\over n_{Pop}}\left(\#(G_1G_1) * a +  \#(G_1G_2) * d + \#(G_2G_2)*(-a)\right)$\\

    \vspace*{1ex} wobei $n_{Pop}$: totale Populationsgr\"osse und \\
    $\#(G_iG_j)$: Anzahl Tiere mit Genotyp $G_iG_j$

  \item Mit $p$ und $q$ als Frequenzen der Allele $G_1$ und $G_2$
  \end{itemize}
...
\frame{
  ...
  Aufgrund der Genotypfrequenzen und dem Hardy-Weinberg-Gleichgewicht folgt
...
\frame{
  ...
  \begin{itemize}
  \item Populationsmittel ($\mu = E(g)$) gibt Informationen zur Lage einer Population
  \item Varianz gibt an, wie gross Streuung um Populationsmittel
  \end{itemize}
}
\note{}
