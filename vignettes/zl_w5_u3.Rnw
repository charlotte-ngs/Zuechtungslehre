...
### # liste zu recodieren der Genotypen auf Anzahl A-Allele
lRecodeMap <- list(AA = 2, AB=1, BB=0)
### # Umkodierung
...
vecBetaTrue <- c(-10.76, 5, 3, 1.5)   ### # vektor der wahren Effekte
matX <- matrix(data = cbind(rep(1, length(locus1)),locus1,locus2,locus3), ncol = 4)
y <- matX %*% vecBetaTrue + rnorm(n=length(locus1), mean = 2, sd=5.5)
