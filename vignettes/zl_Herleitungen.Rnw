...
Das hier gezeigte Material basiert auf folgenden Quellen
\begin{itemize}
\item Skript der Vorlesung Quantitative Genetik der ETHZ \cite{SH2015}
\item Diss Andreas Hofer \cite{Hofer1990}
\item Buch Linear Models for the Prediction of Animal Breeding Values von R.A. Mrode \cite{Mrode2005}
\end{itemize}
...
(\ref{eq:VarHMinusI}) wird im ersten Schritt die Definition der Varianz einer Differenz angewendet. Danach
werden die Definitionen von $H$ und $I$ eingesetzt. Aus der Entwicklung der Formeln in Gleichung
(\ref{eq:VarHMinusI}) k\"onnen folgende Beziehungen zur Varianz des Gesamtzuchtwertes, zur Varianz
...
Zum Minimieren von $Var\left( H-I \right)$ muss dieser Ausdruck in Gleichung (\ref{eq:VarHMinusI})
...
Term bei der Ableitung heraus. Der letzte Term in Gleichung \ref{eq:VarHMinusI} zeigt eine lineare
Abh\"angigkeit in $\mathbf{b}$, was bedeutet, dass in der Ableitung noch der Konstante Faktor
$- 2 * \mathbf{G} * \mathbf{v}$ \"ubrig bleibt.

\begin{eqnarray}
\frac{\partial Var\left(H-I\right)}{\partial\mathbf{b}}
...
\begin{eqnarray}
\frac{\partial Var\left(H-I\right)}{\partial\mathbf{b}}
                         &=& 2*\mathbf{P} * \mathbf{b} - 2 * \mathbf{G} * \mathbf{v} = 0 \nonumber\\
2*\mathbf{P} * \mathbf{b}
                         &=& 2 * \mathbf{G} * \mathbf{v}\nonumber\\
...
\log \left(r_{H,I} \right) &=& \log \left(\frac{Cov(H,I)}{\sqrt{Var(I) * Var(H)}} \right)   \nonumber\\
  &=& \log \left( Cov(H,I) \right) - {1 \over 2} \log \left( Var(I) \right)
       - {1 \over 2} \log \left( Var(H) \right)                                             \nonumber\\
...
Bei n\"aherer Betrachtung der Matrix $\mathbf{L}$ fallen zwei Fakten auf.
\begin{enumerate}
\item Wie schon weiter oben erw\"ahnt sind alle Diagonalelemente der Matrix $\mathbf{L}$ gleich $1$. Da aufgrund der Zerlegung in Gleichung (\ref{eq:AnimalBvDecomp}), jeder Zuchtwert eines Tieres $i$ die Mendelian Sampling Komponente $m_i$ selber enth\"alt, folgt daraus, dass das entsprechende Diagonalelement $l_{ii}$ der Matrix $\mathbf{L}$ gleich $1$ sein muss.
\item Die Offdiagonalelemente $l_{ij}$ wobei $i>j$ lassen sich berechnen als
$$l_{ij} = {1\over 2} * (l_{sj} + l_{dj})$$
wobei $s$ und $d$ die Eltern von $i$ sind. Das Element $l_{ij}$ in Matrix $\mathbf{L}$ bezeichnet den Anteil vom Mendelian Samlpling Effekt $m_j$, welcher im Zuchtwert $u_i$ enthalten ist. Zum Beispiel entspricht das Element $l_{53}$ dem Anteil vom Mendelian Sampling Effekt $m_3$, welcher im Zuchtwert $u_5$ enthalten ist. Aufgrund der Zerlegung des Zuchtwertes $u_5$ in
$$u_5 = {1\over 2}\ u_3 + {1\over 2}\ u_4 + m_5 $$
folgt, dass die Anteile von $m_3$ je zur H\"alfte \"uber den Vaterzuchtwert $u_3$ oder \"uber den Mutterzuchtwert $u_4$ zustande kommen. Der Anteil von $m_3$ im Zuchtwert $u_3$ entspricht dem Element $l_{33}$ in der Matrix $\mathbf{L}$ und der Anteil von $m_3$ im Zuchtwert $u_4$ entspricht analog dem Element $l_{43}$. Somit l\"asst sich das Element $l_{53}$ in unserem Beispiel berechnen als
$$l_{53} = {1\over 2} * l_{33} + {1\over 2} * l_{43} = {1\over 2} * 1 + {1\over 2} * 0 = {1\over 2}$$
\end{enumerate}
...
\subsubsection{Cholesky Zerlegung von $\mathbf{A}$}
Alternativ zum Aufstellen der gesamten Verwandtschaftsmatrix $\mathbf{A}$ k\"onnen die Inzuchtkoeffizienten aufgrund von einzelnen Zeilen der Matrix aus der Cholesky-Zerlegung von $\mathbf{A}$ berechnet werden. Die Cholesky-Zerlegung von $\mathbf{A}$ sieht wie folgt aus
...
Die Gleichung (\ref{eq:DiagElemMatA}) zeigt den allgemeinen Fall der Berechnung der Diagonalelemente $a_{ii}$ der Verwandtschaftsmatrix $\mathbf{A}$. Das bedeutet, dass die Diagonalelemente der Verwandtschaftsmatrix und somit die Inzuchtkoeffizienten aufgrund einer Zeile aus der Matrix $\mathbf{R}$ berechnet werden k\"onnen. Als n\"achste Frage stellt sich, wie die Elemente der Matrix $\mathbf{R}$ rekursiv berechnet werden k\"onnen, ohne dass wir die ganze Matrix $\mathbf{A}$ oder die ganze Matrix $R$ aufstellen.

\subsubsection{Rekursive Berechnung von $\mathbf{R}$}
Der Schl\"ussel zur Berechnung der Elemente von $\mathbf{R}$ liegt in der Gleichsetzung der Cholesky-Zerlegung der Verwandtschaftsmatrix $\mathbf{A}$ und der LDL-Zerlegung von $\mathbf{A}$. Daraus l\"asst sich folgende Beziehung ableiten.

\begin{eqnarray}
\mathbf{A} = \mathbf{R} * \mathbf{R}^T &=& \mathbf{L} * \mathbf{D} * \mathbf{L}^T
\label{CholDecLdlDec}
\end{eqnarray}

Aufgrund von Gleichung (\ref{CholDecLdlDec}) wollen wir die Matrix $\mathbf{R}$ in Abh\"angigkeit der Matrizen $\mathbf{L}$ und $\mathbf{D}$ ausdr\"ucken, dies deshalb, weil wir die Elemente der Matrizen $\mathbf{L}$ und $\mathbf{D}$ einfach berechnen k\"onnen.

Wir zerlegen die Matrix $\mathbf{R}$ in das Produkt der zwei Matrizen $\mathbf{L}$ und $\mathbf{S}$. Somit haben wir

\begin{equation}
\mathbf{R} = \mathbf{L} * \mathbf{S}
\label{eq:RLSDecomp}
\end{equation}
\noindent wobei $\mathbf{S}$ eine Diagonalmatrix ist.

Betrachten wir die Zerlegung in Gleichung (\ref{eq:RLSDecomp}) anhand eines kleinen Beispiels, dann sieht das wie folgt aus

\begin{equation}
<<RlsDecompEx, echo=FALSE, results=tex>>=
...
Aufgrund der speziellen Struktur der Matrizen $\mathbf{L}$ und $\mathbf{S}$ ist ersichtlich, dass die Diagnoalelemente $r_{ii}$ der Matrix $\mathbf{R}$ den Diagonalelementen $s_{ii}$ der Matrix $\mathbf{S}$ entsprechen oder anders formuliert gilt f\"ur alle $i$, dass
...
Die Offdiagonalelemente $r_{ij}$ der Matrix $\mathbf{R}$ lassen sich als Produkt von $l_{ij}$ und $s_{jj}$ berechnen. Als Gleichung k\"onnen wir schreiben, dass
...
\noindent Im Abschnitt \ref{sec:RekursiveZerlegung} haben wir gesehen, dass die Elemente $l_{ij}$ als Durchschnitt von $l_{sj}$ und $l_{dj}$ berechnet werden k\"onnen. Dabei wurde angenommen, dass $s$ und $d$ die Eltern von $i$ sind. Verwenden wir die Berechnung von $l_{ij}$ f\"ur die Berechnung von $r_{ij}$ dann erhalten wir

\begin{eqnarray}
r_{ij} &=& l_{ij} * s_{jj} \nonumber\\
       &=& {1\over 2}\ (l_{sj} + l_{dj}) * s_{jj}\nonumber\\
       &=& {1\over 2}\ (l_{sj} * s_{jj} + l_{dj} * s_{jj})\nonumber\\
       &=& {1\over 2}\ (r_{sj} + r_{dj})
\label{eq:RijDijSjj2}
\end{eqnarray}

Somit ist die rekursive Berechnung der Elemente in Matrix $\mathbf{R}$ bestimmt. Wir m\"ussen jetzt nur noch die Elemente der Matrix $\mathbf{S}$ finden. Dazu setzen wir die Zerlegung der Matrix $\mathbf{R}$ in die Cholesky-Zerklagung der Matrix $\mathbf{A}$ ein und setzen diese der LDL-Zerlegung von Matrix $\mathbf{A}$ gleich, dann folgt, dass

\begin{eqnarray}
\mathbf{A} = \mathbf{R} * \mathbf{R}^T &=& \mathbf{L} * \mathbf{S} * (\mathbf{L}* \mathbf{S})^T \nonumber\\
                                       &=& \mathbf{L} * \mathbf{S} * \mathbf{S}^T * \mathbf{L}^T\nonumber\\
                                       &=& \mathbf{L} * \mathbf{D} * \mathbf{L}^T
\label{eq:FindMatS}
\end{eqnarray}

Da wir auf der linken und auf der rechten Seite des letzten Gleichheitszeichens in (\ref{eq:FindMatS}) von links mit $\mathbf{L}$ und von rechts mit $\mathbf{L}^T$ multiplizieren, k\"onnen wir diese beiden Matrix-Faktoren auch streichen und es gilt also, dass

$$\mathbf{D} = \mathbf{S} * \mathbf{S}^T$$

Zur Veranschaulichung betrachten wir auch dieses Resultat in unserem kleinen Beispiel.

\begin{equation}
<<DequalsSSt, echo=FALSE, results=tex>>=
matD <- matDiag(psBaseElement = "d", pnNrRow = nAnzAni, pnNrCol = nAnzAni)
cat("\\left[")
cat(paste(sGetTexMatrix(pmatAMatrix = matD), collapse = "\n"))
cat("\\right]  = \n")
cat("\\left[")
cat(paste(sGetTexMatrix(pmatAMatrix = matS), collapse = "\n"))
cat("\\right] * \n")
...
Aufgrund der Regeln der Matrixmultiplikation k\"onnen wir sehen, dass f\"ur alle $i$ gilt, dass
$d_{ii} = s_{ii}^2$ oder $s_{ii} = \sqrt{d_{ii}}$. Die Elemenente $d_{ii}$ k\"onnen wir aufgrund der Gleichung (\ref{eq:DiagElemDii}) berechnen als

\begin{eqnarray}
d_{ii} &=& 0.5 - 0.25(F_s + F_d) \nonumber\\
       &=& 1 - 0.25(a_{ss} + a_{dd})
\label{eq:DiagElemDii2}
\end{eqnarray}

Die Diagonalelemente $a_{ss}$ und $a_{dd}$ der Verwandtschaftsmatrix k\"onnen wir aufgrund der Gleichung (\ref{eq:DiagElemMatA}) als Summen von quadrierten Elementen der Matrix $\mathbf{R}$ ausdr\"ucken. Es gilt also

\begin{eqnarray}
a_{ss} &=& \sum_{n=1}^s r_{sn}^2\nonumber\\
a_{dd} &=& \sum_{n=1}^d r_{dn}^2\nonumber
\end{eqnarray}

Die Elemente $r_{sn}$ und $r_{dn}$ werden aufgrund der Gleichungen (\ref{eq:RiiSii}) und (\ref{eq:RijDijSjj2}) berechnet. Somit schliesst sich der Kreis und es konnte gezeigt werden, wie die Inzuchtkoeffizienten aufgrund einer Zeile der Matrix $\mathbf{R}$ berechnet werden konnten.

\textbf{Wichtig} f\"ur diese Herleitung ist, dass die Tiere gem\"ass ihres Alters sortiert sind. Konkret heisst das, dass im Pedigree kein Nachkomme vor seinen Eltern auftreten darf. Die gleiche Sortierung gilt auch f\"ur die hier verwendeten Matrizen. Nur aufgrund dieser Sortierung ist die ganze Erkl\"arung nicht zirkul\"ar, d.h. die Katze beisst sich nicht in den Schwanz.
...
\bibitem{SH2015} Heidi Signer-Hasler und Hannes J\"org (2015). Quantitative Genetik und Populationsgenetik. Skript Vorlesung 751-6200-00 V. FS2015.
\bibitem{Hofer1990} Hofer, Andreas (1990). Sch\"atzung von Zuchtwerten feldgepr\"ufter Schweine mit einem Mehrmerkmals-Tiermodell. Diss ETH 9160. http://dx.doi.org/10.3929/ethz-a-000578289
\bibitem{Mrode2005} R.A. Mrode (2005). Linear Models For The Prediction Of Animal Breeding Values. Second Edition. CABI Publishing. ISBN 0 85199 000 2
\end{thebibliography}
