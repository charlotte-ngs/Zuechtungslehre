...
Zuchtwerte werden mit der Variablen ``$u$'' bezeichnet. Dabei wird der Vektor der Zuchtwerte einer Population von Tieren mit dem kleinen  $\mathbf{u}$ in fetter Schrift bezeichnet. Der Zuchtwert $u_i$ eines einzelnen Tieres $i$ wird mit einem kleinen $u$ in Normalschrift mit einem Subskript $i$ gekennzeichnet.
