...
Im Gegensatz zu Abschnitt \ref{sec:OneLocusModel}, welcher Zuchtwerte an einem bestimmten Genort betrachtete, wechseln wir
...
betrachteten Effekte einer Normalverteilung folgen, kann der Zuchtwert eines Tieres $i$ mit bekannten Eltern $s$ und $d$
zerlegt werden in
...
\noindent wobei $m_i$ f\"ur den sogenannten ``Mendelian sampling'' - Effekt steht. Dieser Effekt entspricht der Abweichung des Zuchtwertes $a_i$ vom erwarteten Vollgeschwisterdurchschnitt. Er kommt durch die zuf\"allige Auswahl von Allelen w\"ahrend der Meiose bei den Eltern zustande. Die Zuchtwerte von Vollgeschwister unterscheiden sich also nur in den unterschiedlichen
Mendelian sampling Effekten, nicht aber in den Beitr\"agen der Elternzuchtwerte.

Fasst man die Zuchtwerte $a_i$ aller Tiere zu einem Vektor $\mathbf{a}$ zusammen, dann kann die Zerlegung aus Gleichung
...
\vspace{1ex}
\begin{tabular}{lll}
wobei  &   $\mathbf{a}$  &  Vektor aller Zuchtwerte \\
...
Die Matrixschreibweise der Zerlegung in Gleichung (\ref{eq:ExPedBvDecomp}) zeigt, dass die Zuchtwerte f\"ur Tiere ohne Eltern den Mendelian sampling Effekten entsprechen. In unserem Beispielpedigree haben die Tiere $1$, $2$ und $4$ keine bekannten Eltern. F\"ur deren Zuchtwerte gilt also, dass $a_1 = m_1$, $a_2 = m_2$ und $a_4 = m_4$.

\subsection{Rekursive Zerlegung}
\label{sec:RekursiveZerlegung}
Kehren wir nun zurueck zur Zerlegung f\"ur ein bestimmtes Tier $a_i$. Wie aus Gleichung (\ref{eq:AnimalBvDecomp}) ersichtlich ist, wird $a_i$ in je die H\"alfte der Elternzuchtwerte $a_s$ und $a_d$ und die Mendelian sampling Komponente zerlegt. Die Zuchtwerte $a_s$ und $a_d$ k\"onnen analog zur Zerlegung von $a_i$ auch in Kompoenten der Elternzuchtwerte und Mendelian sampling aufgeteilt werden. Konkret bedeutet das, dass $a_s$ aufgeteilt wird in

$$a_s = {1\over 2}\ a_{ss} + {1\over 2}\ a_{sd} + m_s$$

und $a_d$ wird zerlegt in

$$a_d = {1\over 2}\ a_{ds} + {1\over 2}\ a_{dd} + m_d$$

\noindent wobei $a_{ss}$ und $a_{sd}$ die Zuchtwerte der Eltern $sd$ und $ss$ von Vater $s$ und $a_{ds}$ und $a_{dd}$ die
Zuchtwerte der Eltern $ds$ und $dd$ der Mutter $d$ sind.

Diese Art der rekursiven Zerlegung der Zuchtwerte wird so lange gemacht bis wir im Pedigree bei Tieren ohne Eltern angelangt sind. Deren Zuchtwerte entsprechen den entsprechenden Mendelian sampling Effekten. Somit kann jeder Zuchtwert $a_i$ als eine
...
a_5 &=& {1\over 2} a_3 + {1\over 2} a_4 + m_5 \\
    &=& {1\over 2}\left({1\over 2}\ a_1 + {1\over 2}\ a_2 + m_3 \right) + {1\over 2}\ m_4 + m_5 \nonumber \\
...
Berechnen wir die Varianz des Vektors der Zuchtwerte $\mathbf{a}$ aufgrund der Gleichung (\ref{eq:ALMDecomp}),so folgt
daraus

\begin{eqnarray}
  Var(\mathbf{a}) &=& Var(\mathbf{L} * \mathbf{m}) \\
                  &=& \mathbf{L} * Var(\mathbf{m}) * \mathbf{L}^T \nonumber
\label{eq:VarAVarD}
\end{eqnarray}

\noindent Eine weiterf\"uhrende Erkl\"arung zur Bedeutung der Varianz von Vektoren ist im Anhang \ref{sec:VarVec} aufgef\"uhrt. Die Varianz der Zuchtwerte $\mathbf{a}$ entspricht der additiv genetischen Covarianz-Matrix und diese ist gleich

\begin{equation}
  Var(\mathbf{a}) = \mathbf{A} * \sigma_{a}^2
\label{eq:AddGenCovMat}
\end{equation}

\noindent wobei $\sigma_a^2$ der additiv genetischen Varianz entspricht.
...
\begin{equation}
  Var(\mathbf{a}) = \mathbf{A} * \sigma_{a}^2
                  = \mathbf{L} * Var(\mathbf{m}) * \mathbf{L}^T
                  = \mathbf{L} * \mathbf{D} * \mathbf{L}^T  * \sigma_{a}^2
\label{eq:VarDAddGenCovMat}
\end{equation}

Aus Gleichung (\ref{eq:VarDAddGenCovMat}) folgt also, dass $Var(\mathbf{m}) = \mathbf{D} * \sigma_a^2$ ist. Da die Mendelian sampling Effekte unabh\"angig sind voneinander, d.h. $Cov(m_i,m_j) = 0$ f\"ur alle $i\ne j$, ist Matrix $\mathbf{D}$ eine Diagonalmatrix. Die Diagonalelemente $d_{ii}$ der Matrix $\mathbf{D}$ entsprechen den Varianzen der jeweiligen Mendelian sampling Effekte, wobei gilt, dass $d_{ii} = Var(m_i)$

% cont here %%%

\subsection{Inverse Verwandtschaftsmatrix}
Die Inverse $\mathbf{A}^{-1}$ der Verwandtschaftsmatrix $\mathbf{A}$ kann anhand der LDL-Zerlegung einfach berechnet werden. Die LDL-Zerlegung f\"ur $\mathbf{A}$ lautet
...
Die Inverse $\mathbf{A}^{-1}$ kann somit bestimmt werden als

\begin{eqnarray}
\mathbf{A}^{-1} &=& \left(\mathbf{L} * \mathbf{D} * \mathbf{L}^{T} \right) \\
                &=& \left(\mathbf{L}^{T} \right)^{-1} * \mathbf{D}^{-1} * \mathbf{L}^{-1} \nonumber
\label{eq:AinvLdl}
\end{eqnarray}

\subsubsection{Matrix $\mathbf{D}^{-1}$}
Die Matrix $\mathbf{D}^{-1}$ ist genau wie die Matrix $\mathbf{D}$ wieder eine Diagonalmatrix, wobei die Diagonalelemente der Inversen $\mathbf{D}^{-1}$ die Werte $1/d_{ii}$ haben.

\subsubsection{Matrix $\mathbf{L}^{-1}$}
Die Struktur der Matrix $\mathbf{L}^{-1}$ kann aufgrund folgender Beziehungen erl\"autert werden. Zu Beginn dieses Abschnittes hatten wir in Gleichung (\ref{eq:PopBvDecomp}) gesehen, wie der Zuchtwert eines Tieres in je die H\"alfte der Zuchtwerte der Eltern plus den Mendelian sampling Effket zerlegt werden kann. In Matrixschreibweise lautet diese Zerlegung
...
\begin{equation}
  \mathbf{m} = \mathbf{a} - \mathbf{P} * \mathbf{a} = \left(\mathbf{I} - \mathbf{P}\right) * \mathbf{a}
\label{eq:MsParentBv}
\end{equation}

\noindent Aufgrund der rekursiven Zerlegung der Zuchtwerte $\mathbf{a}$ in eine Linearkombination von Mendelian sampling Effekten
...
\begin{equation}
  \mathbf{m} = \mathbf{L}^{-1} * \mathbf{a}
\label{eq:MsEffBvDecomp}
\end{equation}

\noindent Durch Gleichsetzen beider Beziehgungen f\"ur $\mathbf{m}$ k\"onnen wir $\mathbf{L}^{-1}$ bestimmen.

\begin{equation}
  \mathbf{m} = \mathbf{L}^{-1} * \mathbf{a} = \left(\mathbf{I} - \mathbf{P}\right) * \mathbf{a}
\label{eq:LinvEq}
\end{equation}
