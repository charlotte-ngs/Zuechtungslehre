...
Aus der Beziehung der beiden Zerlegungen in Gleichung (\ref{CholDecLdlDec}) wollen wir die Matrix $\mathbf{R}$ in Abh\"angigkeit der Matrizen $\mathbf{L}$ und $\mathbf{D}$ ausdr\"ucken. Der Grund daf\"ur ist, dass wir wissen wie wir einfach die Elemente der Matrizen $\mathbf{L}$ und $\mathbf{D}$ bestimmen. Somit wissen wir auch, wie wir die Elemente der Matrix $\mathbf{R}$ bestimmen, sobald wir die Beziehung zwischen $\mathbf{L}$, $\mathbf{D}$ und $\mathbf{R}$ kennen.

Als Idee f\"ur die Bestimmung der Beziehung zwischen $\mathbf{L}$, $\mathbf{D}$ und $\mathbf{R}$ zerlegen wir die Matrix $\mathbf{R}$ in das Produkt der zwei Matrizen $\mathbf{L}$ und $\mathbf{S}$. Somit ist
...
Somit ist die rekursive Berechnung der Elemente in Matrix $\mathbf{R}$ bestimmt. Wir m\"ussen jetzt nur noch finden, wie wir die Elemente der Matrix $\mathbf{S}$ ermitteln. Dazu setzen wir die Zerlegung der Matrix $\mathbf{R}$ in die Cholesky-Zerklagung der Matrix $\mathbf{A}$ ein und setzen diese der LDL-Zerlegung von Matrix $\mathbf{A}$ gleich, dann folgt, dass
