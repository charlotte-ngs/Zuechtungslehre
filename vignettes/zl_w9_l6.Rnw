...
#' Paste a string to a component of a character matrix
#'
...
#' Wrapper for matPasteStringtoMatrixComp
#'
#' \code{matAddStringToMatrixComp} adds a number as a string
#' to a component of a matrix. This is done using the more
#' generic function matPasteStringToMatrixComp
#'
#' @param pmMat         input character matrix
#' @param psPasteToComp   string to be added to matrix component
#' @param pnRowIdx      row index of matrix component where string should be added
...
#' Wrapper for matPasteStringtoMatrixComp
#'
#' \code{matSubStringFromMatrixComp} subtracts a number as a string
#' from a component of a matrix. This is done using the more
#' generic function matPasteStringToMatrixComp
#'
#' @param pmMat         input character matrix
#' @param psPasteToComp   string to be added to matrix component
...
#' Add contributions of a pedigree member to the inverse of the relationship matrix
#'
#' \code{matAddContribToAinv} adds the contribution of a given individual to the
#' inverse relationship matrix.
#'
#' @param pmatAInv   inverse relationship matrix before adding the contribution
#' @param pnAniIdx   index of animal, must be consistent with index in pmatAInv
#' @param pnSireIdx  index of animals sire
#' @param pnDamIdx   index of animals dam
#' @param pnAlpha    alpha value to be added as a contribution for pnAniIdx
#' @return matResultAInv   inverse relationship matrix after adding contribution fo pnAniIdx
matAddContribToAinv <- function(pnAniIdx, pnSireIdx = NA, pnDamIdx = NA, pnAlpha, pmatAInv){
  # initialize result with matrix passed as parameter
  matResult <- pmatAInv
  # add contribution based on pnAniIdx
  matResult <- matAddStringToMatrixComp(pmMat = matResult,
                                        psPasteToComp = round(pnAlpha, digits = 2),
                                        pnRowIdx = pnAniIdx,
                                        pnColIdx = pnAniIdx)
  # if sire is known add contribution between pnAniIdx and sire
  if (!is.na(pnSireIdx)){
    matResult <- matSubStringFromMatrixComp(pmMat = matResult,
                                            psPasteToComp = round(pnAlpha/2, digits = 2),
                                            pnRowIdx = pnAniIdx,
                                            pnColIdx = pnSireIdx)
    matResult <- matSubStringFromMatrixComp(pmMat = matResult,
                                            psPasteToComp = round(pnAlpha/2, digits = 2),
  ...
  # if dam is known, add contribution between pnAniIdx and dam
  if (!is.na(pnDamIdx)){
    matResult <- matSubStringFromMatrixComp(pmMat = matResult,
                                            psPasteToComp = round(pnAlpha/2, digits = 2),
                                            pnRowIdx = pnAniIdx,
  ...
  # if both sire and dam are known add contribution between both of them
  if (!is.na(pnSireIdx) & !is.na(pnDamIdx)) {
    matResult <- matAddStringToMatrixComp(pmMat = matResult,
                                          psPasteToComp = round(pnAlpha/4, digits = 2),
                                          pnRowIdx = pnSireIdx,
                                          pnColIdx = pnSireIdx)
    matResult <- matAddStringToMatrixComp(pmMat = matResult,
                                          psPasteToComp = round(pnAlpha/4, digits = 2),
  ...
  # all contributions added, return result
  return(matResult)
}
@
...
\subsection*{In einem Schritt mit der neuen Funktion}

<<NewAddContribFunction>>=
matAInvA2 <- matrix("0", nrow = nAnzTiere, ncol = nAnzTiere)
alphaA2 <- 1/Dmat(pedA2)
# loop over pedigree
for (nPedIdx in 1:nAnzTiere) {
  matAInvA2 <-  matAddContribToAinv(pnAniIdx = nPedIdx,
                                    pnSireIdx = pedA2@sire[nPedIdx],
                                    pnDamIdx = pedA2@dam[nPedIdx],
                                    pnAlpha = alphaA2[nPedIdx],
                                    pmatAInv = matAInvA2)
}
@

<<NewAinvDisplay, echo=FALSE, results=tex>>=
cat("$$\\mathbf{A}^{-1} = \\left[")
cat(paste(sGetTexMatrix(pmatAMatrix = matAInvA2), collapse = "\n"))
cat("\\right] $$\n")
@

Die Matrix mit den ausgewerteten Summen

<<NewAinvEval, echo=FALSE, results=tex>>=
cat("$$\\mathbf{A}^{-1} = \\left[")
cat(paste(sGetTexMatrix(pmatAMatrix = mParseEval(pmStringMat = matAInvA2)), collapse = "\n"))
cat("\\right] $$\n")
@
